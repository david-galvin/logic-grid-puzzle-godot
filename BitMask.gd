...
func _build_bit_masks(category_size: int):
	for _row in range(category_size):
		for _col in range(category_size):
			true_bit_masks = BitSet.new(max_possible_solutions_per_grid)
	...
	for solution_id in range(math.factorial(category_size)):
		...
		for row in range(category_size):
			...
			true_bit_masks[row][col].set_at_index(solution_id)
